...
data_model = pd.read_csv(
    f'gs://connect-x-production.appspot.com/Organizes/pJoo5lLhhAbbofIfYdLz/AI/model/df_associations.csv'
)

df_art_recomendation = pd.read_csv(
    f'gs://connect-x-production.appspot.com/Organizes/pJoo5lLhhAbbofIfYdLz/AI/model/recommended.csv'
)
# model_names = [
...
@app.get("/v1/get_related/")
async def get_related_v1(url: str, cookie: str):
    db = firestore.Client()
    prod_ref = db.collection(u'Organizes/pJoo5lLhhAbbofIfYdLz/objects/krungsriProduct/data')
    ...
    input_url = unquote(input_url)
    text = input_url.split('/')[-1]
    df_query = data_model.loc[data_model.antecedents.str.contains(text)]
    ...
    while True:
        for cons in consequents:
            ...
            # print(url)
            query = prod_ref.where(u'url', u'==', url).get()
            if query:
                q = query[0].to_dict()
                del q['createdBy']
                del q['lastModified']
        ...
        if result['related_product'].__len__() == 3:
            ...
            lst = related_similarity_content_tfidf(data_model, text, df_art_recomendation)
            df_rec = df_art_recomendation.loc[df_art_recomendation.index == int(lst[count])]
